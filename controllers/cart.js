...
const patch_item = expressasynhandler(async(req,res)=>{
    const user_id = req.user.id;
...
const delete_cart = expressasynhandler(async(req,res)=>{
    const user_id = req.user.id;
    const check_user = await usermodel.findById(user_id);
    if(!check_user) {
        return res.status(404).json({message: "User not found"});
...
module.exports = {cart, patch_item, delete_item, delete_cart, add_item};
